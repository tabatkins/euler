...
problems.p11 = function() {
	...
	for(var row = 0; row < 20; row++) {
		for(var col = 0; col < 16; col++) {
			for(var pathi in paths) {
				var path = paths[pathi];
				if(!isValidSquarePath([row,col], path, 20, 20)) continue;
				var product = path.map(function(offset) {
					return grid[row+offset[0]][col+offset[1]];
				}).reduce(op.mul, 1);
				max = Math.max(max, product);
			}
		}
